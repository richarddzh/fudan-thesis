...
\usepackage[slantfont,boldfont]{xeCJK} %% 不再使用CJKnumber选项
\usepackage{zhnumber} %% 用来取代CJKnumb宏包
\usepackage[bookmarksnumbered]{hyperref}
\usepackage{graphicx}
\usepackage{titlesec} %% 用来产生中文章节样式
\usepackage{titletoc,tocbibind} %% MARK 增加两个package来处理目录
%\usepackage[titles]{tocloft} %% MARK 不再使用tocloft产生目录中的中文章节样式
\usepackage[numbers,sort&compress]{natbib} %% 根据使用gb7714.bst的要求添加
\usepackage{pdfpages} %% 用来嵌入封面和封底的pdf

\defaultfontfeatures{Mapping=tex-text} %% 是tex命令忽略双引号、连字符等
